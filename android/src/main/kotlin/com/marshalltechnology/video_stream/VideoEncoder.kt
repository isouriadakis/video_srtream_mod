...
class VideoEncoder(
    ...
    var now : ZonedDateTime = ZonedDateTime.now(ZoneId.of( "America/Montreal" ) ) 
    ...
    fun start() {
        ...
        now = ZonedDateTime.now(ZoneId.of( "America/Montreal" )) 
        //ZonedDateTime startOfToday = now.toLocalDate().atStartOfDay(ZoneId.of( "America/Montreal" )) ;
    ...
    protected fun sendBuffer(byteBuffer: ByteBuffer,
                             bufferInfo: MediaCodec.BufferInfo) {
        Log.e(TAG, "timestamp " + (Instant.now()) + " " + ZonedDateTime.now() +" " + ZonedDateTime.now(ZoneId.systemDefault())  )

        bufferInfo.presentationTimeUs = (ZonedDateTime.now(ZoneId.of("UTC")).get(ChronoField.MILLI_OF_DAY)).toLong()*1000//Duration.between(now.toLocalDate().atStartOfDay(ZoneId.of( "America/Montreal" )),now).toMillis()
